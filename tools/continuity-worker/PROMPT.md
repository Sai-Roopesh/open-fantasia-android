...
- Reuse a stable existing `cast_id` and `entity_id` when identity matches. Every Cast Member must reference exactly one entry in `world_state.entity_state`. For a genuinely new person, create both the world entity and `cast:<normalized-name>:<first_seen_turn_id>`, using a reachable retained exchange as `first_seen_turn_id`. Never silently merge uncertain duplicates; preserve both until a person can resolve them.
