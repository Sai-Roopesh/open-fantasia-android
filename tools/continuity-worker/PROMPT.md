...
Read `request.json`. Return only one JSON object matching the response envelope described below. Do not use markdown fences.

Produce a complete new `world_state`, not a mutation list. Use only the previous complete snapshot and the exact exchanges supplied—there is intentionally no full transcript.
...
- Reuse a stable existing `cast_id` and `entity_id` when identity matches. For a genuinely new person, use `cast:<normalized-name>:<first_seen_turn_id>` and link the matching world entity when one exists. Never silently merge uncertain duplicates; preserve both until a person can resolve them.
...
When `trigger_reason` is `rewind`, later exchanges were deliberately pruned. They are not supplied and must remain completely unknowable: rebuild continuity only from the retained baseline and retained exchanges. Use `old_head_turn_id` and `discarded_exchange_count` only as proof that pruning occurred; never infer or recreate discarded events.
...
- Return `timeline_events` for genuinely notable beats in the supplied exchanges: reveals, betrayals, discoveries, combat, scene changes, time skips, major relationship shifts, significant emotional turns, or meaningful movement. Routine dialogue gets no event.
